...
How WakuWaku keeps CPU flat while a provider streams, what each piece of the
...
The root `WakuWaku` view re-renders on every frame regardless of what is dirty, so
