...
WakuWaku auto-updates with [Sparkle](https://sparkle-project.org). Releases live in
a **Cloudflare R2** bucket served at **`https://releases.bingzi.dev`**. New users
...
`https://releases.bingzi.dev/appcast.xml`, verifies each build's EdDSA signature,
...
- Updater code: [`src/updater.rs`](src/updater.rs) — loads the embedded
  Sparkle.framework at runtime and starts `SPUUpdater` with WakuWaku's custom user
...
**This Mac already has the key** — WakuWaku signs with the same default-account
...
Sparkle tools (they land in `.wakuwaku-cache/sparkle/<version>/bin` after any
...
To split WakuWaku onto its own key later: `generate_keys --account wakuwaku`, put the
new public key in Info.plist, and pass `--account wakuwaku` through to
...
profile with `--notary-profile` / `WAKUWAKU_NOTARY_PROFILE`.

### 3. Cloudflare R2 bucket + domain  ← **still to do once**

1. Create the bucket **`wakuwaku-releases`** (Cloudflare dashboard → R2 → Create
   bucket). The release script will not create it — a bucket-scoped API token
   can't.
2. Attach the custom domain **`releases.bingzi.dev`** to the bucket (bucket →
   Settings → Custom Domains). This serves objects publicly at
   `https://releases.bingzi.dev/<file>`.
3. Make sure the R2 API token behind the `r2` rclone remote covers this bucket
   ...
   for kero; if `rclone lsf r2:wakuwaku-releases --s3-no-check-bucket` returns
...
the bundled JS REPL, builds the styled DMG, notarizes
...
- **Download link**: `https://releases.bingzi.dev/WakuWaku-<version>.dmg`
...
- `WakuWaku-<version>.dmg`
- `WakuWaku-<version>.zip`
- `appcast.xml` (Sparkle-signed)

Linux CI adds:

- `wakuwaku-<version>-x86_64-unknown-linux-gnu.tar.gz`
- `wakuwaku-<version>-aarch64-unknown-linux-gnu.tar.gz`
...
distribution WakuWaku can start on (2.35 — Ubuntu 22.04, Debian 12, Fedora 36).
...
`https://wakuwaku.bingzi.dev/install.sh` — see [docs/linux.md](docs/linux.md).

Publishing that GitHub release (or running **Sync release** from Actions)
uploads the assets to the `wakuwaku-releases` R2 bucket. Configure these repository
secrets first:

| Secret | Purpose |
| --- | --- |
| `WAKUWAKU_ANALYTICS_ENDPOINT` | embedded in the macOS CI build |
| `WAKUWAKU_ANALYTICS_WEBSITE_ID` | embedded in the macOS CI build |
| `WAKUWAKU_SIGNING_IDENTITY` | Developer ID identity selector |
...
| `R2_BUCKET` | optional; defaults to `wakuwaku-releases` |
...
| `--build-number <n>` / `WAKUWAKU_BUILD_NUMBER` | derived | `CFBundleVersion` override |
| `WAKUWAKU_R2_REMOTE` | `r2` | rclone remote name |
| `WAKUWAKU_R2_BUCKET` | `wakuwaku-releases` | R2 bucket |
| `WAKUWAKU_DOWNLOAD_URL_PREFIX` | `https://releases.bingzi.dev/` | base URL in the appcast |
| `WAKUWAKU_HISTORY_COUNT` | `15` | recent archives pulled for delta generation |
| `WAKUWAKU_NO_HISTORY=1` | — | skip pulling old archives (full updates only) |
| `SPARKLE_BIN` | the `.wakuwaku-cache` copy | Sparkle tools directory |
...
- **Debug builds never update themselves.** `Updater::init` returns `None`
  ...
  itself with a production WakuWaku. Set `WAKUWAKU_FORCE_UPDATER=1` to exercise the
  ...
  start the watcher with `WAKUWAKU_PREVIEW_UPDATE=1`; the sidebar immediately
...
- **WakuWaku isn't sandboxed**, so Sparkle's XPC services are unnecessary;
...
- **Platform artifacts:** keep the bucket layout flat and platform-tagged by
  ...
  (`WakuWaku-<v>.dmg`, `WakuWaku-<v>.zip`, `appcast.xml`) must keep their URLs.
  Linux CI releases produce `wakuwaku-<v>-<target>.tar.gz` with
  ...
  `WakuWaku-<v>-Setup.exe` + `appcast-windows.xml` (WinSparkle reads the same
