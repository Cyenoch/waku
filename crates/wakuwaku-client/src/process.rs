...
impl DaemonProcess {
    ...
    fn spawn_configured(
        ...
        let app_executable =
            std::env::current_exe().context("could not locate WakuWaku executable")?;
        ...
        let stdout = child
            .stdout
            .take()
            .context("WakuWaku daemon did not expose its readiness stream")?;
        let (ready_tx, ready_rx) = mpsc::sync_channel(1);
        std::thread::Builder::new()
            .name("wakuwaku-daemon-ready".into())
            .spawn(move || {
                ...
                let result = BufReader::new(stdout)
                    ...
                    .and_then(|bytes| {
                        if bytes == 0 {
                            bail!("WakuWaku daemon exited before becoming ready")
                        }
                        serde_json::from_str::<DaemonReady>(&line).map_err(anyhow::Error::from)
                    });
                let _ = ready_tx.send(result);
            })
            .context("could not start WakuWaku daemon readiness reader")?;
        let ready = match ready_rx.recv_timeout(START_TIMEOUT) {
            ...
            Err(error) => {
                let _ = child.kill();
                let _ = child.wait();
                bail!("timed out waiting for WakuWaku daemon: {error}");
            }
        };
...
fn desktop_client_address(address: &str) -> anyhow::Result<String> {
    let address = address
        ...
        .with_context(|| format!("WakuWaku daemon returned an invalid address {address:?}"))?;
...
impl DaemonSupervisor {
    ...
    pub fn spawn_configured(
        ...
        std::thread::Builder::new()
            .name("wakuwaku-daemon-supervisor".into())
            .spawn(move || monitor_daemon(weak_inner, initial_stamp, watch_for_rebuilds))
            .context("could not start WakuWaku daemon supervisor")?;
        Ok(supervisor)
    }
    ...
    fn from_target(
        ...
        std::thread::Builder::new()
            .name("wakuwaku-daemon-settings".into())
            .spawn(move || persist_settings(weak_inner, settings_update_rx))
            .context("could not start WakuWaku daemon settings writer")?;
        Ok(Self { inner })
    }
    ...
    pub fn reconfigure(&self, exposure: DaemonExposureSettings) -> anyhow::Result<()> {
        ...
        let executable = self
            ...
            .executable
            .as_ref()
            .context("the connected daemon is managed outside WakuWaku Desktop")?
            .clone();
...
fn monitor_daemon(
    ...
    loop {
        ...
        match replace_local_daemon(&inner, executable, &exposure) {
            Ok(()) => {}
            Err(error) => {
                eprintln!("could not restart rebuilt WakuWaku daemon: {error:#}");
                continue;
            }
        }
    ...
    let previous = {
        ...
        match &*target {
            DaemonTarget::Remote(_) => {
                bail!("the connected daemon is managed outside WakuWaku Desktop")
            }
    ...
    while let Ok(mut settings) = updates.recv() {
        ...
        loop {
            ...
            match result {
                ...
                Ok(_) => {
                    eprintln!("WakuWaku daemon returned an invalid settings update response");
                }
                Err(error) => {
    ...
    fn browser_origins_are_exact_and_deduplicated() {
        assert_eq!(
            parse_allowed_origins(
                "https://app.wakuwaku.test, http://localhost:3001, https://app.wakuwaku.test"
            )
            .unwrap(),
            ["https://app.wakuwaku.test", "http://localhost:3001"]
        );
        assert!(parse_allowed_origins("https://app.wakuwaku.test/path").is_err());
        assert!(parse_allowed_origins("ws://app.wakuwaku.test").is_err());
    }
