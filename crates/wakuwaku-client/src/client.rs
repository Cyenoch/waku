...
impl DaemonClient {
    ...
    pub fn connect_with_resume(
        ...
        let (mut socket, _) =
            tungstenite::client::connect_with_config(url.as_str(), Some(config), 3)
                .context("could not connect to WakuWaku daemon")?;
        ...
        std::thread::Builder::new()
            .name("wakuwaku-daemon-client".into())
            .spawn(move || run_client(socket, outgoing_rx, thread_inner))
            .context("could not start WakuWaku daemon client thread")?;
        Ok(Self { inner })
    }
    ...
    pub fn request(
        ...
        if self.inner.disconnected.load(Ordering::Acquire) {
            bail!("WakuWaku daemon is disconnected");
        }
            ...
            .is_err()
        {
            ...
            bail!("WakuWaku daemon connection is closed");
        }
        match response_rx.recv_timeout(REQUEST_TIMEOUT) {
            ...
            Err(error) => {
                self.inner.pending.lock().remove(&request_id);
                Err(anyhow!("timed out waiting for WakuWaku daemon: {error}"))
            }
        }
    }
    ...
    pub fn notify(
        ...
        self.inner
            ...
            .map_err(|_| anyhow!("WakuWaku daemon connection is closed"))
    }
...
fn daemon_url(address: &str) -> anyhow::Result<String> {
    ...
    let mut url = url::Url::parse(&normalized).context("WakuWaku daemon address is invalid")?;
    url.set_path("/v1");
    url.set_query(None);
...
fn run_client(
                ...
                match message {
                    ...
                    ServerMessage::Event(event) => {
                        ...
                        if should_deliver {
                            ...
                            if let Some(subscribers) = sessions.get_mut(&key) {
                                subscribers.retain(|events| events.send(event.clone()).is_ok());
    ...
    for (_, response) in pending {
        let _ = response.send(Err(RpcError {
            message: "WakuWaku daemon disconnected".into(),
        }));
    }
    let sessions = std::mem::take(&mut *inner.sessions.lock());
    for ((session_id, runtime_id), subscribers) in sessions {
        ...
        for events in subscribers {
            let _ = events.send(exited.clone());
        }
    }
...
fn read_server_message(
    ...
    loop {
        match socket.read()? {
            ...
            Message::Close(_) => bail!("WakuWaku daemon closed during handshake"),
            _ => {}
        }
    }
}
