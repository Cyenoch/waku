...
impl ComposerDraftStore {
    ...
    pub fn load(&self) -> io::Result<ComposerDrafts> {
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "WakuWaku daemon returned an invalid composer-drafts response",
            )),
        }
    }
    ...
    pub fn save(&self, drafts: ComposerDrafts, generation: u64) -> io::Result<()> {
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "WakuWaku daemon returned an invalid composer-drafts save response",
            )),
        }
    }
}
...
impl StateStore {
    ...
    pub fn session_message_search(
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "WakuWaku daemon returned an invalid message-search response",
            )),
        }
    }
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
            ...
            .map_err(to_io_error)?
        {
            ResponsePayload::TaskState {
                ...
                wakuwaku_protocol::projectless::set_workspace_root(projectless_root);
                (projects, sessions, default_cwd)
            }
            _ => {
                return Err(io::Error::other(
                    "WakuWaku daemon returned an invalid task-state response",
                ));
            }
        };
    ...
    pub fn save(&self, state: &mut PersistedState) -> io::Result<()> {
        ...
        self.daemon
            ...
            .notify(
                Uuid::nil(),
                Uuid::nil(),
                Command::SaveTaskState(Box::new(wakuwaku_protocol::SaveTaskState {
...
pub fn hydrate_session(
        ...
        .map_err(to_io_error)?
    {
        ...
        _ => Err(io::Error::other(
            "WakuWaku daemon returned an invalid session-hydration response",
        )),
    }
}
...
pub fn read_remote_reference(
    ...
    let command = if wakuwaku_protocol::blob::is_reference(reference) {
        Command::ReadBlob {
            reference: reference.to_owned(),
        }
    } else if reference.starts_with(wakuwaku_protocol::attachments::ATTACHMENT_SCHEME) {
