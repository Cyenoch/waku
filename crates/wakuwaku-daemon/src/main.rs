...
use wakuwaku_protocol::{DAEMON_TOKEN_ENV, DaemonReady, PROTOCOL_VERSION};

fn main() -> anyhow::Result<()> {
    let arguments = Arguments::parse(std::env::args().skip(1))?;
    let token = std::env::var(DAEMON_TOKEN_ENV)
        .context("WakuWaku daemon authentication token is missing")?;
    ...
    let listener = TcpListener::bind(&arguments.bind)
        .with_context(|| format!("could not bind WakuWaku daemon to {}", arguments.bind))?;
    ...
    if let Some(parent_pid) = arguments.parent_pid {
        ...
        std::thread::Builder::new()
            .name("wakuwaku-daemon-parent".into())
            .spawn(move || {
    ...
    let task_path = wakuwaku_core::persistence::StateStore::default_path();
    let settings =
        wakuwaku_core::DaemonSettingsStore::open(wakuwaku_core::DaemonSettings::default_path())
            .context("could not load daemon settings")?;
    let task_store = wakuwaku_core::persistence::StateStore::daemon(task_path);
    wakuwaku_core::serve(
        listener,
        token,
        Arc::new(wakuwaku_core::daemon::WakuBackend::new(
            settings, task_store,
        )?),
        shutdown,
        wakuwaku_core::ServerOptions {
...
mod tests {
    ...
    fn parses_repeated_browser_origin_allowlist_entries() {
        ...
        assert_eq!(
            ...
            ["https://app.wakuwaku.test", "http://localhost:3000"]
        );
