...
use wakuwaku_harness::{Harness, RunOutcome};
...
fn provider_config(base_url: &str, format: ApiFormat, auth: Auth) -> ProviderConfig {
    ProviderConfig {
        endpoint: wakuwaku_provider::ExternalProvider {
            id: wakuwaku_provider::ProviderId::new("test-provider"),
...
fn context_with_image() -> PromptContext {
    PromptContext {
        system_prompt: None,
        messages: vec![Message::User(wakuwaku_harness::UserMessage {
            parts: vec![
                wakuwaku_harness::UserPart::Text("look".into()),
                wakuwaku_harness::UserPart::Image {
...
async fn retry_after_retries_429_and_caps_server_delay_without_leaking_key() {
    ...
    .await;
    let retry = wakuwaku_harness::RetryPolicy {
        max_attempts: 2,
...
async fn http_errors_redact_configured_extra_header_values() {
    ...
    let provider = http_provider(config).with_retry(wakuwaku_harness::RetryPolicy {
        max_attempts: 1,
        base_delay: Duration::from_millis(1),
        max_retry_after: Duration::from_millis(1),
    });
...
impl Tool for DelayTool {
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            self.calls.fetch_add(1, Ordering::SeqCst);
            Ok(ExecOutcome {
                parts: vec![wakuwaku_harness::ToolResultPart::Text(self.name.into())],
                details: None,
...
async fn agent_tool_finished_carries_exact_success_and_failure_results() {
    ...
    assert!(matches!(
        ...
        [wakuwaku_harness::ToolResultPart::Text(text)] if text == "fast"
    ));
    ...
    assert!(matches!(
        ...
        [wakuwaku_harness::ToolResultPart::Text(text)] if text.contains("unknown tool: missing")
    ));
...
fn built_in_tools_execute_and_reject_path_traversal() {
    ...
    let write = wakuwaku_harness::WriteTool::unbound();
    let read = wakuwaku_harness::ReadTool::unbound();
    let edit = wakuwaku_harness::EditTool::unbound();
    let list = wakuwaku_harness::ListTool::unbound();
    let search = wakuwaku_harness::SearchTool::unbound();
    let shell = wakuwaku_harness::ShellTool::unbound().with_timeout(Duration::from_secs(2));
    ...
    .unwrap();
    assert!(
        matches!(&write_result.parts[0], wakuwaku_harness::ToolResultPart::Text(text) if text.contains("wrote"))
    );
    ...
    .unwrap();
    assert!(
        matches!(&read_result.parts[0], wakuwaku_harness::ToolResultPart::Text(text) if text.contains("alpha") && text.contains("beta"))
    );
    run_tool(
        &edit,
        &context,
    ...
    .unwrap();
    assert!(
        matches!(&list_result.parts[0], wakuwaku_harness::ToolResultPart::Text(text) if text.contains("file.txt"))
    );
    ...
    .unwrap();
    assert!(
        matches!(&search_result.parts[0], wakuwaku_harness::ToolResultPart::Text(text) if text.contains("file.txt"))
    );
    ...
    .unwrap();
    assert!(
        matches!(&shell_result.parts[0], wakuwaku_harness::ToolResultPart::Text(text) if text.contains("shell-ok"))
    );
    ...
    #[cfg(unix)]
    {
        ...
        let outside = root
            .parent()
            .unwrap()
            .join(format!("wakuwaku-harness-outside-{}", std::process::id()));
