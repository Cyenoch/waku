    ...
    for msg in repaired_messages(&ctx.messages).iter() {
        match msg {
            ...
            Message::Assistant(a) => {
                ...
                let has_signed_thinking = a.content.iter().any(|b| {
                    matches!(
                        b,
                        ContentBlock::Thinking(t)
                            if t.signature.as_deref().is_some_and(|signature| !signature.trim().is_empty())
                                && !t.thinking.is_empty()
                    )
                });
                if text.is_empty() && tool_calls.is_empty() && !has_signed_thinking {
                    continue;
                }
