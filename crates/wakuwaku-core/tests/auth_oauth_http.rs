...
use wakuwaku_protocol::model::{InteractionMode, RuntimeMode};
...
use wakuwaku_provider::{apply_opencode_route, apply_xai_policy};
...
struct LiveSession {
    mock: Arc<MockHttp>,
    handle: DriverHandle,
    rx: crossbeam_channel::Receiver<wakuwaku_protocol::model::DriverEvent>,
...
impl LiveSession {
    fn start(
        ...
        let directory = std::env::temp_dir().join(format!("wakuwaku-{label}-{}", Uuid::new_v4()));
...
fn codex_browser_callback_exchanges_and_refreshes_on_injected_token_host() {
    ...
    let port = mock.bind();
    let directory = std::env::temp_dir().join(format!("wakuwaku-codex-http-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let store = Arc::new(MemoryCredentialStore::default());
    let service = AuthService::new(AuthRuntime::testing(
        &directory,
        Arc::clone(&store) as Arc<dyn wakuwaku_core::auth::CredentialStore>,
    ...
    let (auth, extra) = service
        ...
        .unwrap();
    assert!(matches!(auth, wakuwaku_harness::Auth::Bearer(_)));
    assert!(
        extra
            .iter()
...
fn codex_device_pending_then_exchanges_authorization_code() {
    ...
    let port = mock.bind();
    let directory = std::env::temp_dir().join(format!("wakuwaku-codex-dev-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let service = AuthService::new(AuthRuntime::testing(
        &directory,
...
fn xai_device_pending_slow_down_refresh_and_invalid_grant() {
    ...
    let directory = std::env::temp_dir().join(format!("wakuwaku-xai-http-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let store = Arc::new(MemoryCredentialStore::default());
    ...
    let (auth, extra) = service
        ...
        .unwrap();
    assert!(matches!(auth, wakuwaku_harness::Auth::Bearer(token) if token == "xai-access-2"));
...
fn client_status_returns_browser_phase_and_busy_device_fallback() {
    ...
    let directory = std::env::temp_dir().join(format!("wakuwaku-phase-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let settings = DaemonSettingsStore::open(directory.join("settings.json")).unwrap();
...
fn opencode_go_apply_switches_responses_to_anthropic_without_service_tier() {
    let session = LiveSession::start(
        ...
        Some(wakuwaku_protocol::ServiceTier::Priority),
        &[
        ...
        |base| {
            ...
            assert_eq!(
                ...
                wakuwaku_protocol::ApiFormat::Anthropic
            );
    ...
    assert!(
        session
            .handle
            .apply_options(wakuwaku_client::driver::SessionOptions {
                mode: RuntimeMode::FullAccess,
                interaction_mode: InteractionMode::Build,
                model: Some("claude-sonnet-4-5".into()),
                reasoning_effort: None,
                service_tier: Some(wakuwaku_protocol::ServiceTier::Priority),
                context_window: None,
            })
    );
...
fn openai_official_chat_sends_requested_service_tier() {
    let session = LiveSession::start(
        ...
        Some(wakuwaku_protocol::ServiceTier::Flex),
...
fn expired_oauth_refreshes_before_second_prompt() {
    ...
    let port = mock.bind();
    let directory = std::env::temp_dir().join(format!("wakuwaku-refresh-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
    let creds = Arc::new(MemoryCredentialStore::default());
    ...
    wakuwaku_core::auth::AuthRuntime::testing(
        &persist_dir,
        Arc::clone(&creds) as Arc<dyn wakuwaku_core::auth::CredentialStore>,
        endpoints.clone(),
    )
    .persist
    .put_catalog(
...
fn get_auth_status_completes_xai_device_and_seeds_catalog() {
    ...
    let directory =
        std::env::temp_dir().join(format!("wakuwaku-xai-status-poll-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let store = Arc::new(MemoryCredentialStore::default());
    let mut endpoints = injected_codex_endpoints(port);
    endpoints.xai_discovery = format!("http://127.0.0.1:{port}/.well-known/openid-configuration");
...
fn wait_connected(rx: &crossbeam_channel::Receiver<wakuwaku_protocol::model::DriverEvent>) {
    wait_event(rx, |event| {
        matches!(event, wakuwaku_protocol::model::DriverEvent::Connected)
    });
}

fn wait_finished(rx: &crossbeam_channel::Receiver<wakuwaku_protocol::model::DriverEvent>) {
    wait_event(rx, |event| {
        matches!(
            event,
            wakuwaku_protocol::model::DriverEvent::TurnFinished { .. }
        )
    });
}
...
fn wait_event(
    rx: &crossbeam_channel::Receiver<wakuwaku_protocol::model::DriverEvent>,
    mut pred: impl FnMut(&wakuwaku_protocol::model::DriverEvent) -> bool,
) {
