...
use wakuwaku_protocol::composer::{CommandScope, FileEntry, SlashCommand};
...
pub fn discover_slash_commands(_provider: ProviderId, project_root: &Path) -> Vec<SlashCommand> {
    ...
    scan_command_files(
        &project_root.join(".wakuwaku/commands"),
        CommandScope::Project,
        &mut commands,
    );
    if let Some(home) = dirs::home_dir() {
        scan_command_files(
            &home.join(".config/wakuwaku/commands"),
            CommandScope::User,
            &mut commands,
        );
    }
...
mod tests {
    ...
    fn generic_commands_are_available_without_provider_catalogs() {
        let root = std::env::temp_dir().join(format!("wakuwaku-composer-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join(".wakuwaku/commands")).unwrap();
        std::fs::write(root.join(".wakuwaku/commands/deploy.md"), "Deploy safely").unwrap();
