...
use crate::protocol::{
    ...
    ResponsePayload, RpcError, SequencedEvent, SequencedPayload, ServerMessage, WireDriverEvent,
};
...
pub trait Backend: Send + Sync + 'static {
    ...
    fn bind_event_hub(&self, _hub: EventHub) {}
}
...
/// Cloneable handle for post-commit trajectory live updates.
#[derive(Clone)]
pub struct EventHub {
    hub: Arc<Hub>,
}
...
impl EventHub {
    pub fn emit_trajectory(
        ...
        self.hub.emit(
            session_id,
            Uuid::nil(),
            SequencedPayload::Trajectory { update },
            true,
        );
    }
}
...
impl Hub {
    ...
    fn emit(
        ...
        let mut state = self.state.lock();
        let runtime_id = match &payload {
            SequencedPayload::Trajectory { .. } => state
                .active_runtimes
                .get(&session_id)
                .copied()
                .unwrap_or(Uuid::nil()),
            SequencedPayload::Driver { .. } => runtime_id,
        };
        let allowed = match &payload {
            SequencedPayload::Trajectory { .. } => {
                runtime_id.is_nil() || state.active_runtimes.get(&session_id) == Some(&runtime_id)
            }
            SequencedPayload::Driver { .. } => {
                state.active_runtimes.get(&session_id) == Some(&runtime_id)
            }
        };
        if !allowed {
            return;
...
impl RequestDispatcher {
    ...
    fn dispatch_independent(
        ...
        if let Err(error) = std::thread::Builder::new()
            .name("wakuwaku-daemon-request".into())
            .spawn(move || {
    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("wakuwaku-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
...
pub fn serve(
    ...
    listener
        ...
        .context("could not configure WakuWaku daemon listener")?;
    let hub = Arc::new(Hub::default());
    backend.bind_event_hub(EventHub { hub: hub.clone() });
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("wakuwaku-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("wakuwaku-daemon connection ended: {error:#}");
                        }
                    })
                    .context("could not start WakuWaku daemon connection thread")?;
            }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                std::thread::sleep(ACCEPT_POLL_INTERVAL);
            }
            Err(error) if error.kind() == io::ErrorKind::Interrupted => {}
            Err(error) => return Err(error).context("WakuWaku daemon listener failed"),
        }
    }
    backend.shutdown();
    Ok(())
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("wakuwaku-daemon ignored invalid message: {error}");
                }
            },
            ...
            Err(error) => return Err(error).context("WakuWaku daemon WebSocket failed"),
        }
    }
...
mod tests {
    ...
    fn driver_kind(event: &SequencedEvent) -> &str {
        event.driver().map(|wire| wire.kind.as_str()).unwrap_or("")
    }

    fn driver_payload(event: &SequencedEvent) -> &serde_json::Value {
        &event.driver().expect("expected a driver payload").payload
    }
        ...
        fn handle(&self, request: Request, _events: EventSink) -> anyhow::Result<ResponsePayload> {
            match request.command {
                ...
                Command::LoadTaskState => Ok(ResponsePayload::TaskState {
                    ...
                    projectless_root: Some(PathBuf::from("/tmp/.wakuwaku/projects")),
                }),
                _ => Ok(ResponsePayload::Ack),
            }
        }
    ...
    fn stale_projection_cannot_resurrect_a_removed_session() {
        let root = std::env::temp_dir().join(format!("wakuwaku-remove-race-{}", Uuid::new_v4()));
    ...
    fn late_client_attaches_to_replay_and_live_runtime_events() {
        ...
        source
            ...
            .unwrap();
        assert_eq!(
            driver_kind(&source_events.recv_timeout(Duration::from_secs(1)).unwrap()),
            "connected"
        );
        ...
        assert_eq!(driver_kind(&replayed), "connected");
        ...
        source
            .request(
                ...
                runtime_id,
                Command::Prompt {
                    input: wakuwaku_protocol::PromptInput::text("streamed from the first client"),
                },
            )
            .unwrap();
        for events in [&source_events, &late_events] {
            ...
            assert_eq!(driver_kind(&live), "textDelta");
            assert_eq!(
                driver_payload(&live),
                &json!("streamed from the first client")
            );
        }
    ...
    fn websocket_terminal_round_trip_streams_input_and_output() {
        let root = std::env::temp_dir().join(format!("wakuwaku-terminal-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let backend = WakuBackend::new(
            DaemonSettingsStore::open(root.join("settings.json")).unwrap(),
            StateStore::daemon(root.join("app.db")),
            ...
            && !output.windows(marker.len()).any(|window| window == marker)
        {
            ...
            seen_events.push(driver_kind(&event).to_owned());
            if driver_kind(&event) != "terminalOutput" {
                continue;
            }
            let data = driver_payload(&event)["data"].as_str().unwrap();
            output.extend(
    ...
    fn browser_origins_are_denied_unless_explicitly_allowed() {
        let request = HandshakeRequest::builder()
            .uri("/v1")
            .header(ORIGIN, "https://app.wakuwaku.test")
            .body(())
            .unwrap();
        ...
        let allowed = HashSet::from(["https://app.wakuwaku.test".to_owned()]);
    ...
    fn replaced_runtime_ignores_late_events_from_the_old_generation() {
        ...
        assert_eq!(event.sequence, 1);
        assert_eq!(driver_kind(&event), "new");
        ...
        dispatcher.dispatch(
            Request {
                ...
                command: Command::Prompt {
                    input: wakuwaku_protocol::PromptInput::text("after start"),
                },
            },
    ...
    #[test]
    fn query_trajectory_does_not_target_a_runtime_mailbox() {
        assert!(!command_targets_runtime(&Command::QueryTrajectory {
            query: wakuwaku_protocol::TrajectoryQuery::Page {
                before: None,
                limit: Some(10),
    ...
    #[test]
    fn hub_sequences_trajectory_updates_on_the_active_runtime() {
        let hub = Arc::new(Hub::default());
        let session_id = Uuid::new_v4();
        let runtime_id = Uuid::new_v4();
        let (outgoing, events) = unbounded();
        hub.subscribe(&[], outgoing);
        hub.begin_runtime(session_id, runtime_id);
        EventHub { hub: hub.clone() }.emit_trajectory(
            session_id,
            wakuwaku_protocol::TrajectoryLiveUpdate::Reset {
                generation: 2,
                revision: 9,
            },
        );
        let ServerMessage::Event(event) = events.recv().unwrap() else {
            panic!("expected a sequenced event");
        };
        assert_eq!(event.runtime_id, runtime_id);
        assert_eq!(event.sequence, 1);
        assert!(matches!(
            event.payload,
            SequencedPayload::Trajectory {
                update: wakuwaku_protocol::TrajectoryLiveUpdate::Reset {
                    generation: 2,
                    revision: 9,
                }
            }
    ...
    #[test]
    fn hub_accepts_nil_runtime_trajectory_when_no_runtime_is_live() {
        let hub = Arc::new(Hub::default());
        let session_id = Uuid::new_v4();
        let (outgoing, events) = unbounded();
        hub.subscribe(&[], outgoing);
        EventHub { hub }.emit_trajectory(
            session_id,
            wakuwaku_protocol::TrajectoryLiveUpdate::Reset {
                generation: 1,
                revision: 1,
        ...
        assert!(event.runtime_id.is_nil());
        assert_eq!(event.sequence, 1);
        assert!(matches!(event.payload, SequencedPayload::Trajectory { .. }));
    }
