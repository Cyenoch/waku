...
use uuid::Uuid;
pub use wakuwaku_protocol::settings::DaemonSettings;
        ...
        let settings = match fs::read(&path) {
            Ok(bytes) => match serde_json::from_slice(&bytes) {
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "WakuWaku daemon moved invalid settings to {}: {error}",
                        backup.display()
                    );
...
mod tests {
    ...
    fn corrupt_current_settings_are_quarantined_and_replaced() {
        let directory = std::env::temp_dir().join(format!("wakuwaku-settings-{}", Uuid::new_v4()));
    ...
    fn replace_rejects_reserved_and_invalid_endpoints() {
        ...
        let reserved = DaemonSettings {
            external_providers: vec![wakuwaku_protocol::ExternalProvider::new(
                wakuwaku_protocol::ProviderId::OPENAI_CHAT,
                "Nope",
                "http://127.0.0.1:9/v1",
