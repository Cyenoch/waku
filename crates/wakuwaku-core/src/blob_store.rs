...
//! the transcript keeps only a `wakuwaku-blob:` reference.
...
pub use wakuwaku_protocol::blob::SCHEME as BLOB_SCHEME;
...
pub fn is_blob_reference(value: &str) -> bool {
    wakuwaku_protocol::blob::is_reference(value)
}
...
impl BlobStore {
    ...
    /// Resolves a `wakuwaku-blob:` reference against this daemon store's root.
...
mod tests {
    ...
    fn temporary_root() -> PathBuf {
        std::env::temp_dir().join(format!("wakuwaku-blobs-{}", uuid::Uuid::new_v4()))
    }
    ...
    fn traversal_references_are_rejected() {
        let store = BlobStore::new(temporary_root());
        assert!(store.path_for("wakuwaku-blob:../../etc/passwd").is_none());
