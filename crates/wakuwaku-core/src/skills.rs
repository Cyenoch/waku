...
//! project's `.agents/skills` / `.wakuwaku/skills` trees (see
...
pub use wakuwaku_protocol::skills::{
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("wakuwaku-skills-{tag}-{}", std::process::id()));
    ...
    fn every_ecosystem_root_is_listed() {
        ...
        for expected in ["/tmp/waku/.agents/skills", "/tmp/waku/.wakuwaku/skills"] {
            assert!(
