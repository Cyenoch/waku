...
const TURN_START_METADATA_PREFIX: &str = "WakuWaku-Turn-Start: ";
...
pub fn checkpoint_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/wakuwaku/session-{session_id}-turn-{turn_count}")
}

pub fn turn_start_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/wakuwaku/session-{session_id}-turn-start-{turn_count}")
}

pub fn turn_diff_base_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/wakuwaku/session-{session_id}-turn-diff-{turn_count}")
}
...
pub fn capture_turn(cwd: &Path, session_id: Uuid, turn_count: usize) -> anyhow::Result<Checkpoint> {
    ...
    let end_commit =
        capture_worktree_commit_from(cwd, end_head.as_deref(), "WakuWaku worktree snapshot", &[])?;
...
pub fn capture_worktree_commit(cwd: &Path) -> anyhow::Result<String> {
    ...
    let head = resolve_ref(cwd, "HEAD");
    capture_worktree_commit_from(cwd, head.as_deref(), "WakuWaku worktree snapshot", &[])
}
...
fn capture_worktree_commit_from(
    ...
    let temporary_index = common_dir.join(format!("wakuwaku-checkpoint-index-{}", Uuid::new_v4()));
...
fn virtual_branch_start(
    ...
    commit_tree(cwd, tree, "WakuWaku turn diff base", &[])
}

fn empty_tree_commit(cwd: &Path) -> anyhow::Result<String> {
    commit_tree(cwd, EMPTY_TREE, "WakuWaku empty turn diff base", &[])
}
...
fn session_checkpoint_ref_commits(cwd: &Path, session_id: Uuid) -> SessionCheckpointRefs {
    let prefix = format!("refs/wakuwaku/session-{session_id}-");
    git_output(
        cwd,
        [
            "for-each-ref",
    ...
    S: AsRef<OsStr>,
{
    ...
    if identity {
        command
            .env("GIT_AUTHOR_NAME", "WakuWaku")
            .env("GIT_AUTHOR_EMAIL", "waku@localhost")
            .env("GIT_COMMITTER_NAME", "WakuWaku")
...
mod tests {
    ...
    fn diverged_repository() -> PathBuf {
        let directory =
            std::env::temp_dir().join(format!("wakuwaku-checkpoints-{}", Uuid::new_v4()));
