...
use wakuwaku_protocol::provider::ProviderId;

pub use wakuwaku_protocol::usage_history::{
...
pub fn enumerate_days(since_day: NaiveDate, until_day: NaiveDate) -> Vec<NaiveDate> {
    wakuwaku_protocol::usage_history::enumerate_days(since_day, until_day)
}

pub fn first_of_month(day: NaiveDate) -> NaiveDate {
    wakuwaku_protocol::usage_history::first_of_month(day)
}

pub fn enumerate_months(since_day: NaiveDate, until_day: NaiveDate) -> Vec<NaiveDate> {
    wakuwaku_protocol::usage_history::enumerate_months(since_day, until_day)
}

pub fn days_in_month(first_day: NaiveDate) -> u32 {
    wakuwaku_protocol::usage_history::days_in_month(first_day)
}
...
mod tests {
    ...
    fn rate_tables_parse_and_round_trip_through_the_disk_cache() {
        ...
        let dir = std::env::temp_dir().join(format!("wakuwaku-usage-rates-{}", std::process::id()));
