...
//!
//! New chats live under `~/.wakuwaku/projects/<local date>/<prompt slug>`, with
...
pub fn set_workspace_root(root: Option<PathBuf>) {
    wakuwaku_protocol::projectless::set_workspace_root(root.clone());
...
pub fn create_workspace(prompt: Option<&str>) -> io::Result<Workspace> {
    let root = workspace_root().ok_or_else(|| {
        io::Error::new(
            ...
            "could not locate the home directory for ~/.wakuwaku/projects",
        )
    })?;
...
mod tests {
    ...
    fn test_root() -> PathBuf {
        std::env::temp_dir().join(format!("wakuwaku-projectless-{}", Uuid::new_v4()))
    }
