...
//! worktree beneath `~/.wakuwaku/worktrees`, then every workspace consumer uses
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.wakuwaku/worktrees"))?
        .join(".wakuwaku/worktrees");
    create_in(
        project_path,
        &root,
        project_id,
...
mod tests {
    ...
    fn creates_isolated_worktrees_from_the_default_branch() {
        let root = std::env::temp_dir().join(format!("wakuwaku-worktree-test-{}", Uuid::new_v4()));
