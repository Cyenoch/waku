...
pub fn parse_models_payload(
    preset: Option<ProviderPreset>,
    payload: &Value,
    provider: waku_provider::ProviderId,
    base_url: &str,
    format: waku_provider::ApiFormat,
    ...
    let models = match preset {
        ...
        _ if format == waku_provider::ApiFormat::Anthropic => {
            parse_anthropic_models_envelope(payload, provider, base_url)?
        }
    ...
    if config.transport == TransportProfile::Codex {
        ...
        headers.insert(
            ...
            reqwest::header::HeaderValue::from_static(
                waku_provider::AuthEndpoints::CODEX_ORIGINATOR,
            ),
        );
        headers.insert(
            "version",
...
mod tests {
    use super::*;
    use serde_json::json;
    use waku_provider::{ApiFormat, ProviderId};
    ...
    fn codex_models_url_includes_canonical_client_version() {
        ...
        .unwrap();
        let version = waku_provider::AuthEndpoints::client_version();
