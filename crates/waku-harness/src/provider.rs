...
pub struct ProviderConfig {
    pub endpoint: waku_provider::ExternalProvider,
    pub auth: Auth,
    pub transport: waku_provider::TransportProfile,
...
impl ProviderConfig {
    pub fn new(endpoint: waku_provider::ExternalProvider, auth: Auth) -> Self {
        Self {
            endpoint,
            auth,
...
fn route_and_auth_headers(
    ...
    if provider.transport == waku_provider::TransportProfile::Codex {
        headers.insert(
            "openai-beta",
        ...
        headers.insert(
            "originator",
            header_value(waku_provider::AuthEndpoints::CODEX_ORIGINATOR, "originator")?,
        );
        headers.insert(
            "version",
            header_value(waku_provider::AuthEndpoints::client_version(), "version")?,
        );
    }
...
mod tests {
    ...
    fn provider(format: ApiFormat, auth: Auth) -> ProviderConfig {
        ProviderConfig {
            endpoint: waku_provider::ExternalProvider {
                id: waku_provider::ProviderId::new("p"),
                name: "P".into(),
    ...
    fn codex_transport_sets_subscription_headers() {
        ...
        p.transport = waku_provider::TransportProfile::Codex;
        ...
        assert_eq!(
            ...
            waku_provider::AuthEndpoints::client_version()
        );
