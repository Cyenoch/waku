...
use uuid::Uuid;
use waku_harness::Auth;
use waku_protocol::xai_oauth_seed;
use waku_protocol::{
    ...
    pub fn resolve(
            ...
            && !value.trim().is_empty()
        {
            let auth = match endpoint.api_format {
                waku_protocol::ApiFormat::Anthropic => Auth::AnthropicApiKey {
                    key: value,
...
fn capabilities_for_preset(
    ...
    format: waku_protocol::ApiFormat,
) -> ModelCapabilities {
    match preset {
...
mod tests {
    ...
    use waku_protocol::ModelCapabilities;

    #[test]
    fn env_fallback_does_not_cross_alias_xai() {
        let directory = std::env::temp_dir().join(format!("waku-auth-{}", Uuid::new_v4()));
    ...
    fn go_and_zen_stored_keys_never_cross() {
        let directory = std::env::temp_dir().join(format!("waku-auth-gozen-{}", Uuid::new_v4()));
    ...
    fn transport_failure_keeps_last_good_catalog() {
        let directory = std::env::temp_dir().join(format!("waku-auth-cache-{}", Uuid::new_v4()));
        ...
        persist
            .put_catalog(
                ...
                vec![ModelCatalogEntry {
                    ...
                    id: "grok-4.5".into(),
                    name: "Grok".into(),
                    api_format: waku_protocol::ApiFormat::OpenAiChat,
    ...
    fn api_key_login_never_writes_settings_json() {
        ...
        let directory = std::env::temp_dir().join(format!("waku-auth-settings-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&directory).unwrap();
    ...
    fn browser_callback_is_accepted_between_status_polls() {
        let directory = std::env::temp_dir().join(format!("waku-auth-cb-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&directory).unwrap();
        let store = Arc::new(MemoryCredentialStore::default());
        let service = service_with_store(&directory, store);
        let phase = service
    ...
    fn busy_callback_port_returns_device_fallback() {
        let directory = std::env::temp_dir().join(format!("waku-auth-busy-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&directory).unwrap();
    ...
    fn selected_go_model_missing_from_catalog_is_an_error() {
        let directory = std::env::temp_dir().join(format!("waku-auth-go-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&directory).unwrap();
        let store = Arc::new(MemoryCredentialStore::default());
        store
            .set(
    ...
    fn unsupported_catalog_entry_never_resolves() {
        let directory = std::env::temp_dir().join(format!("waku-auth-unsup-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&directory).unwrap();
        let store = Arc::new(MemoryCredentialStore::default());
        store
            .set(
        ...
        persist
            .put_catalog(
                ...
                vec![ModelCatalogEntry {
                    ...
                    supported: false,
                    unsupported_reason: Some(waku_protocol::UnsupportedReason::GoogleFormat),
                }],
                CatalogSource::Live,
                1,
            )
    ...
    fn api_key_is_rejected_for_codex_and_supergrok() {
        let directory = std::env::temp_dir().join(format!("waku-auth-method-{}", Uuid::new_v4()));
    ...
    fn complete_api_key_requires_matching_awaiting_session() {
        let port = bind_json(200, serde_json::json!({ "data": [{ "id": "grok-4.5" }] }));
        let directory = std::env::temp_dir().join(format!("waku-auth-phase-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&directory).unwrap();
        let mut runtime = AuthRuntime::testing(
            &directory,
            Arc::new(MemoryCredentialStore::default()),
    ...
    fn device_first_poll_honors_interval_on_controllable_clock() {
        let directory = std::env::temp_dir().join(format!("waku-auth-clock-{}", Uuid::new_v4()));
        let service = service_with_store(&directory, Arc::new(MemoryCredentialStore::default()));
    ...
    fn supergrok_oauth_does_not_add_chatgpt_account_header() {
        let directory = std::env::temp_dir().join(format!("waku-auth-hdr-{}", Uuid::new_v4()));
        let store = Arc::new(MemoryCredentialStore::default());
        store
            .set(
    ...
    fn concurrent_provider_logins_stay_self_describing() {
        let directory =
            std::env::temp_dir().join(format!("waku-auth-concurrent-{}", Uuid::new_v4()));
        let service = service_with_store(&directory, Arc::new(MemoryCredentialStore::default()));
        let xai = service
            .start_login(ProviderId::new("xai"), LoginMethod::ApiKey)
            .unwrap();
