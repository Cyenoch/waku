//! In-process HTTP provider driver backed by `waku-harness`.
...
use waku_harness::{AssistantMessage, ContentBlock, RequestOptions};
use waku_harness::{Budget, Harness, RunOutcome, Session};
use waku_protocol::model::{
    ActivityKind, DriverEvent, InteractionMode, PermissionOption, RuntimeMode,
};
use waku_protocol::{ExternalProvider, ProviderId};
...
pub(super) struct EmbeddedDriver {
    ...
    steering: Arc<Mutex<Option<waku_harness::SessionSteering>>>,
...
enum Command {
    Prompt(waku_harness::UserMessage),
    ApplyOptions {
...
struct ConfigInput<'a> {
    ...
    capabilities: waku_protocol::ModelCapabilities,
}
...
impl EmbeddedDriver {
    pub(super) fn start(
        ...
        thread::Builder::new()
            .name(format!("waku-embedded-{}", provider_id.as_str()))
            .spawn(move || {
                worker(Worker {
                    runtime,
                    receiver,
...
impl EmbeddedDriver {
    pub(super) fn prompt(&self, prompt: waku_harness::UserMessage) {
        if self.is_active() {
            self.steer(prompt);
            return;
        }
    ...
    pub(super) fn steer(&self, prompt: waku_harness::UserMessage) {
        let text = waku_harness::UserMessage::text_of(&prompt.parts);
        let handle = self
            .steering
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
    ...
    pub(super) fn respond_user_input(
        &self,
        request_id: String,
        _answers: Vec<waku_protocol::model::UserInputAnswer>,
    ) -> anyhow::Result<()> {
        bail!("no pending user input request {request_id}")
    }

    pub(super) fn reject_background_stop(&self, key: waku_protocol::model::BackgroundWorkKey) {
        let _ = self.events.send(DriverEvent::BackgroundWork(Box::new(
            waku_protocol::model::BackgroundWorkEvent::StopFailed {
                key,
    ...
    pub(super) fn snapshot(&self) -> anyhow::Result<waku_harness::SessionSnapshot> {
        let (response, receiver) = bounded(1);
        self.commands
...
impl EventBridge {
    ...
    fn emit(&mut self, event: AgentEvent) {
        match event {
            // The driver maps a prompt to one visible turn. Harness turn events
            // also include every internal assistant/tool cycle, so publishing
            // them would incorrectly split a single user turn in the UI.
    ...
    fn tool_call(&mut self, call: std::sync::Arc<waku_harness::ToolCall>) {
    ...
    fn tool_finished(&mut self, result: std::sync::Arc<waku_harness::ToolResult>) {
            ...
            .then(|| {
                let text = result
                    ...
                    .filter_map(|part| match part {
                        waku_harness::ToolResultPart::Text(text) => Some(text.as_str()),
                        waku_harness::ToolResultPart::Image { .. } => None,
                    })
                    .collect::<Vec<_>>()
                    .join("\n");
...
mod tests {
    ...
    fn billed_usage() -> waku_harness::Usage {
        waku_harness::Usage {
            input: 4,
            output: 2,
            cache_read: 1,
    ...
    fn done_and_failed_with_tokens_each_emit_one_billed_event() {
        ...
        bridge.emit_stream(StreamEvent::Done {
            usage: billed_usage(),
            stop_reason: waku_harness::StopReason::Stop,
        });
        bridge.emit_stream(StreamEvent::Failed {
            usage: billed_usage(),
            stop_reason: waku_harness::StopReason::Error,
    ...
    fn zero_token_responses_do_not_emit_usage() {
        ...
        bridge.emit_stream(StreamEvent::Done {
            usage: waku_harness::Usage::default(),
            stop_reason: waku_harness::StopReason::Stop,
        });
        assert!(rx.try_recv().is_err());
    }
    ...
    fn tool_loop_done_then_final_done_bills_twice() {
        ...
        bridge.emit_stream(StreamEvent::Done {
            usage: billed_usage(),
            stop_reason: waku_harness::StopReason::ToolUse,
        });
        bridge.emit_stream(StreamEvent::Done {
            usage: billed_usage(),
            stop_reason: waku_harness::StopReason::Stop,
