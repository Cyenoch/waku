...
use waku_core::persistence::StateStore;
...
fn api_key_login_refreshes_models_and_logout_clears_status() {
    ...
    let directory = std::env::temp_dir().join(format!("waku-auth-smoke-{}", Uuid::new_v4()));
    ...
    let ResponsePayload::Login { phase } = client
        .start_login(ProviderId::new("xai"), waku_protocol::LoginMethod::ApiKey)
        .unwrap()
    else {
...
fn oauth_device_start_is_invoked_for_supergrok() {
    let directory = std::env::temp_dir().join(format!("waku-oauth-smoke-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
...
fn get_auth_status_phases_name_their_provider() {
    let directory = std::env::temp_dir().join(format!("waku-auth-status-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let (address, shutdown) = start_daemon(&directory, AuthEndpoints::production(), None);
    let client = DaemonClient::connect(&address, TOKEN.into()).unwrap();
