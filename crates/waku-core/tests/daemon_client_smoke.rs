...
use waku_core::driver::WAKU_SYSTEM_PROMPT;
use waku_core::model::{DriverEvent, InteractionMode, ProviderId, RuntimeMode};
use waku_core::persistence::{PersistedState, StateStore};
use waku_core::protocol::{Command, ResponsePayload};
use waku_core::{DaemonSettings, DaemonSettingsStore, ServerOptions, serve};
use waku_protocol::{ApiFormat, ExternalProvider, PromptImageRef};
...
fn daemon_client_mock_http_smoke() {
    let directory = std::env::temp_dir().join(format!("waku-smoke-{}", Uuid::new_v4()));
    ...
    unsafe {
        std::env::set_var("WAKU_SMOKE_KEY", "sk-smoke");
    }
    ...
    provider.api_key_env = Some("WAKU_SMOKE_KEY".into());
    ...
    handle.prompt(PromptInput {
        ...
        attachments: vec![PromptImageRef::Blob { reference: stored }],
    });
    ...
    assert!(
        snapshot
            .messages
            .iter()
            .any(|message| matches!(message, waku_harness::Message::User(_))),
