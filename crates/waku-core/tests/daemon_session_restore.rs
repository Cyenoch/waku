...
use waku_core::protocol::{Command, ResponsePayload, StartTask};
use waku_core::{DaemonSettings, DaemonSettingsStore, ServerOptions, serve};
use waku_protocol::{ApiFormat, ExternalProvider};
...
fn failed_pre_provider_session(project_id: Uuid, provider: &str) -> waku_core::model::AgentSession {
    let mut session = waku_core::model::AgentSession::new(project_id, ProviderId::new(provider));
    ...
    session.status = waku_core::model::SessionStatus::Failed;
...
fn start_daemon(
    ...
    DaemonClient,
    Uuid,
    waku_core::model::AgentSession,
    Arc<AtomicBool>,
) {
    let mock = MockHttp::new(vec![model_sse("restored")]);
    let port = mock.bind();
    unsafe {
        std::env::set_var("WAKU_RESTORE_KEY", "sk-restore");
    }
    ...
    provider.api_key_env = Some("WAKU_RESTORE_KEY".into());
...
fn persisted_pre_provider_failure_continues_over_mock_http() {
    let directory = std::env::temp_dir().join(format!("waku-restore-{}", Uuid::new_v4()));
    ...
    assert!(
        restored.harness_snapshot(session_id).is_some() || {
...
fn fresh_daemon_db_restores_app_local_session_over_mock_http() {
    let directory = std::env::temp_dir().join(format!("waku-restore-fresh-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
    ...
    let missing = client
        .request(
            ...
            Uuid::new_v4(),
            Command::Start {
                options: waku_core::protocol::WireDriverStartOptions {
                    provider: ProviderId::new(PROVIDER),
                    cwd: workspace.clone(),
                    mode: "ask".into(),
...
fn dropping_a_driver_handle_clone_keeps_prompt_events() {
    let directory = std::env::temp_dir().join(format!("waku-handle-clone-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
    let (client, session_id, session, shutdown) = start_daemon(&directory, &workspace, false);
...
fn start_failure_after_restore_still_has_snapshot() {
    let directory = std::env::temp_dir().join(format!("waku-restore-fail-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
    let settings = DaemonSettingsStore::open(directory.join("settings.json")).unwrap();
...
fn genuine_provider_termination_surfaces_real_failure() {
    let directory = std::env::temp_dir().join(format!("waku-restore-term-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
...
fn divergent_start_generation_does_not_prompt() {
    let directory = std::env::temp_dir().join(format!("waku-restore-gen-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
    let (client, session_id, session, shutdown) = start_daemon(&directory, &workspace, true);
    ...
    let mismatch = error
        .downcast_ref::<waku_client::StartGenerationMismatch>()
...
fn equal_timestamp_stale_client_closes_rejected_runtime() {
    let directory = std::env::temp_dir().join(format!("waku-restore-eq-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let workspace = directory.join("ws");
    std::fs::create_dir_all(&workspace).unwrap();
    ...
    let mut state = PersistedState::fresh(workspace.clone());
    let mut existing =
        waku_core::model::AgentSession::new(state.projects[0].id, ProviderId::new(PROVIDER));
    ...
    existing.finish_active_turn(TurnStatus::Completed);
    existing.status = waku_core::model::SessionStatus::Idle;
    ...
    store.set_harness_snapshot(
        session_id,
        waku_harness::Session::new(Some("waku".into())).snapshot(),
    );
    store.save(&mut state).unwrap();
