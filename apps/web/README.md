...
Browser client for an existing WakuWaku daemon. The Cloudflare Worker serves the
TanStack Start application only; it does not start, proxy, or store credentials
for a daemon.

```sh
bun --filter @wakuwaku/web dev
```
...
```sh
WAKUWAKU_DAEMON_TOKEN=replace-me cargo run -p wakuwaku-daemon --bin wakuwaku-daemon -- \
  --bind 127.0.0.1:34123 \
  --allow-origin http://localhost:3001
```

WakuWaku Desktop can expose the daemon it manages from Settings → Daemon, where you
...
```sh
bun --filter @wakuwaku/web build
bun --filter @wakuwaku/web deploy
```
