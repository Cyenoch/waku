import {
  ...
  formToProvider,
  providerIdIsReferenced,
  providerIdsConflict,
  replaceProviderById,
} from "@/components/settings-view";
import { resolvedSessionModel } from "@/components/model-picker";
...
describe("applyComposerDraftChanges", () => {
  test("sends keyed updates instead of replacing every client draft", async () => {
    ...
    await expect(
      applyComposerDraftChanges(client, changes),
    ...
    const client = {
      request: async (command: unknown) => {
        ...
        return commands.length === 1
          ...
          : { type: "login", phase: { type: "completed", loginId: "login-1" } };
      },
    } as unknown as WakuClient;
    await expect(startLogin(client, "xai", "apiKey")).resolves.toMatchObject({
      type: "awaitingApiKey",
      loginId: "login-1",
    });
    await expect(
      completeApiKeyLogin(client, "login-1", "xai", "secret"),
    ).resolves.toMatchObject({ type: "completed", loginId: "login-1" });
    expect(commands).toEqual([
      { type: "startLogin", provider: "xai", method: "apiKey" },
      {
        type: "completeApiKeyLogin",
        loginId: "login-1",
        provider: "xai",
        key: "secret",
      },
      ...
      models: [
        {
          ...
          name: "Google",
          provider: "opencode-go",
          apiFormat: "openAiChat" as const,
          transport: "standard" as const,
          ...
          supported: false,
          unsupportedReason: "googleFormat" as const,
        },
      ],
    };
    let command: unknown;
...
describe("web auth and catalog guards", () => {
  ...
  test("only allows secure external URLs and localhost HTTP fixtures", () => {
    expect(isAllowedExternalUrl("https://auth.example.test/callback")).toBe(
      true,
    );
    ...
    expect(
      isAllowedExternalUrl("https://user:pass@example.test/callback"),
    ).toBe(false);
  ...
  test("does not keep a Go model on a SuperGrok session", () => {
    ...
    expect(resolvedSessionModel("kimi-k2.7-code", [grok])).toBe(
      "grok-4.5",
    );
    expect(resolvedSessionModel("grok-4.5", [grok])).toBe(
      "grok-4.5",
    );
    expect(
      resolvedSessionModel("kimi-k2.7-code", []),
    ).toBeUndefined();
  });

  test("keeps API formats off the auth-provider list and labels them by translation key", () => {
    expect(BUILTIN_AUTH_PROVIDERS.map((provider) => provider.id)).toEqual([
      "openai-codex",
      "opencode-go",
      "opencode-zen",
      "xai",
    ...
    expect(CUSTOM_PROVIDER_API_FORMATS).toEqual([
    ...
    expect(
      CUSTOM_PROVIDER_API_FORMATS.some((format) =>
        BUILTIN_AUTH_PROVIDERS.some((provider) => provider.id === format.id),
      ),
    ).toBe(false);
  });
});

describe("custom provider identity helpers", () => {
  test("rejects duplicate IDs while allowing the provider being edited", () => {
    const providers = [{ id: "existing" }, { id: "other" }];
    expect(providerIdsConflict(providers, "other", null)).toBe(true);
    expect(providerIdsConflict(providers, "existing", "existing")).toBe(false);
    expect(providerIdsConflict(providers, "openai-responses", null)).toBe(true);
  });

  test("replaces a renamed provider without leaving the old row", () => {
    const existing = { id: "old", name: "Old" } as ExternalProvider;
    const next = { id: "new", name: "New" } as ExternalProvider;
    const result = replaceProviderById([existing], "old", next);
    expect(result.map((provider) => provider.id)).toEqual(["new"]);
  });
  test("detects session, favorite, last-provider, and remembered references", () => {
    const preferences = {
      lastProvider: "other",
      modelTraits: {
        "corp\u0000model": {
    ...
    expect(
      providerIdIsReferenced("corp", [{ provider: "corp" }], [], preferences),
    ).toBe(true);
    expect(
      providerIdIsReferenced("corp", [], ["corp:model"], preferences),
    ).toBe(true);
    expect(
      providerIdIsReferenced("corp", [], [], {
        ...preferences,
        lastProvider: "corp",
      }),
    ).toBe(true);
    expect(providerIdIsReferenced("corp", [], [], preferences)).toBe(true);
    expect(providerIdIsReferenced("unused", [], [], preferences)).toBe(false);
  });
  test("preserves validation errors instead of coercing malformed provider fields", () => {
    const base = {
      id: "corp",
      name: "Corp",
    ...
    expect(formToProvider({ ...base, headers: "malformed" }).error).toBe(
      "providers.invalid_header_format",
    );
    expect(
      formToProvider({ ...base, headers: "Authorization: nope" }).error,
    ).toBe("providers.invalid_headers");
    expect(formToProvider(base).provider).toMatchObject({
      headers: [["X-Test", "ok"]],
    });
  });
});
...
describe("browseDaemonDirectory", () => {
  test("lists an absolute directory on the daemon host", async () => {
    ...
    await expect(browseDaemonDirectory(client, "/Users/me")).resolves.toEqual(
      result,
    );
...
describe("turn checkpoints", () => {
  test("captures the immutable starting ref on the daemon host", async () => {
    ...
    await expect(
      captureTurnStart(client, "/srv/wakuwaku", "session", 2),
    ).resolves.toBeUndefined();
    expect(command).toEqual({
      type: "workspace",
      operation: {
  ...
  test("returns the ending checkpoint captured by the daemon", async () => {
    ...
    await expect(
      captureTurnCheckpoint(client, "/srv/wakuwaku", "session", 2),
...
describe("persistProject", () => {
  ...
  test("reuses a project already persisted for the same daemon path", async () => {
    ...
    const result = await persistProject(
      client,
      project("duplicate", "wakuwaku", "/srv/wakuwaku"),
    );
...
describe("selectableProjects", () => {
  test("represents projectless tasks as one choice while preserving the selected workspace", () => {
    ...
    const first = project(
      "one",
      "No project",
      "/home/me/.wakuwaku/projects/one",
    );
    const selected = project(
      "two",
      "No project",
      "/home/me/.wakuwaku/projects/two",
    );
