...
import { nextModelPickerHighlight, readyCatalogModels, resolvedCatalogModel, selectedModelPickerIndex } from '@/lib/model-picker-presentation'
import { cn } from '@/lib/utils'

type PickerTab = 'favorites' | ProviderId
type PickerModel = Pick<ModelCatalogEntry, 'id' | 'name' | 'supported' | 'unsupportedReason' | 'capabilities' | 'apiFormat' | 'reasoningEfforts' | 'defaultReasoningEffort'> & { source: 'live' | 'cache' | 'seed' }
...
export { resolvedSessionModel } from '@/lib/model-picker-presentation'
...
export function ModelPicker({
  ...
  const readyModels = readyCatalogModels(currentQuery ?? { isError: false, isFetched: false })
  const selectedModels = readyModels?.map((model) => ({
    ...model,
    source: currentQuery?.data?.source ?? 'live',
  }))
  const selectedModel = resolvedCatalogModel(session.model, selectedModels)
  ...
  useEffect(() => {
    if (!readyModels || !selectedModel || selectedModel.id === session.model) return
    onChange(session.provider, selectedModel)
  }, [onChange, readyModels, selectedModel, session.model, session.provider])

  const rows = (() => {
    const normalized = query.trim().toLowerCase()
    const visible = normalized ? providerEntries : providerEntries.filter(({ id }) => tab === 'favorites' || id === tab)
            ...
            <div className="flex min-w-0 flex-1 flex-col bg-card">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-[9px]" ref={list}>
                ...
                {rows.map((row, index) => {
                  ...
                  return <div aria-selected={selected} className={cn('flex h-[58px] w-full items-center gap-2.5 rounded-[9px] border border-transparent px-3 text-left outline-none hover:bg-accent', selected && 'bg-accent', index === highlight && 'border-ring bg-accent')} data-model-index={index} id={`model-${row.provider}-${row.model.id}`} key={`${row.provider}:${row.model.id}`} role="option" tabIndex={0} onClick={() => choose(index)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); choose(index) } }} onMouseEnter={() => setHighlight(index)}>
                    <span className="min-w-0 flex-1"><span className="block truncate text-[13px] font-semibold">{row.model.name}</span><span className="mt-1 flex items-center gap-1.5 truncate text-[11px] text-[var(--text-tertiary)]"><ProviderIcon className="size-[10.5px]" provider={row.provider} />{providerEntries.find((provider) => provider.id === row.provider)?.name || row.provider}<span>· {t(`models.catalog_${row.model.source}`)}</span></span></span>
