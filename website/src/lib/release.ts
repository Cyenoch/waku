...
const RELEASES_BASE = 'https://releases.bingzi.dev'
...
export const FALLBACK_DOWNLOAD_URL = `${RELEASES_BASE}/WakuWaku-0.0.1.dmg`
...
const fetchLatestRelease = createServerFn({ method: 'GET' }).handler(
  async (): Promise<LatestRelease | null> => {
    try {
      ...
      return {
        version,
        url: `${RELEASES_BASE}/WakuWaku-${version}.dmg`,
        pubDate,
      }
    } catch {
      return null
