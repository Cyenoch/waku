...
describe("WakuClient", () => {
  ...
  test("deduplicates events and resumes from the last sequence", async () => {
    ...
    const event = {
      ...
      epoch: "epoch-one",
      sequence: 4,
      payload: { type: "driver", event: { kind: "textDelta", payload: { text: "hi" } } },
    };
    first.receive(event);
    first.receive(event);
  ...
  test("accepts sequence one again when the daemon epoch changes", async () => {
    ...
    socket.receive({
      ...
      epoch: "old",
      sequence: 9,
      payload: { type: "driver", event: { kind: "textDelta", payload: null } },
    });
    socket.receive({
      type: "event",
      sessionId: "session",
  ...
  test("buffers replayed events until a refreshed app attaches to the runtime", async () => {
    ...
    socket.receive({
      ...
      epoch: "epoch",
      sequence: 1,
      payload: { type: "driver", event: { kind: "textDelta", payload: "before attach" } },
    });
    socket.receive({
      type: "event",
      sessionId: "session",
      ...
      epoch: "epoch",
      sequence: 2,
      payload: { type: "driver", event: { kind: "textDelta", payload: "still before attach" } },
    });
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("WakuWaku daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("WakuWaku daemon is disconnected");
  });
  ...
  test("queryTrajectory uses the envelope session id and nil runtime", async () => {
    const { client, sockets } = fixture();
    const socket = await connect(client, sockets);
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const pending = client.queryTrajectory(sessionId, {
      type: "page",
      limit: 25,
      atLeastRevision: 3,
    });
    ...
    expect(request).toMatchObject({
      ...
      command: {
        type: "queryTrajectory",
        query: { type: "page", limit: 25, atLeastRevision: 3 },
      },
    });
    socket.receive({
      type: "response",
    ...
    await expect(pending).resolves.toMatchObject({ type: "page", revision: 3 });
  });

  test("subscribeTrajectory delivers typed live updates and ignores driver payloads", async () => {
    const { client, sockets } = fixture();
    const socket = await connect(client, sockets);
    const received: Array<{ revision: number; sequence: number }> = [];
    client.subscribeTrajectory("session", "runtime", (update, event) => {
      if (update.type === "reset") {
        received.push({ revision: update.revision, sequence: event.sequence });
      }
    });
    socket.receive({
      type: "event",
      ...
      epoch: "epoch",
      sequence: 1,
      payload: { type: "driver", event: { kind: "textDelta", payload: "nope" } },
    });
    socket.receive({
      type: "event",
      sessionId: "session",
      ...
      epoch: "epoch",
      sequence: 2,
      payload: { type: "trajectory", update: { type: "reset", generation: 1, revision: 8 } },
    });
    expect(received).toEqual([{ revision: 8, sequence: 2 }]);
  });
});
