...
export type TrajectoryListener = (update: TrajectoryLiveUpdate, event: SequencedEvent) => void;
...
export class WakuClient {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          failHandshake(new Error("WakuWaku daemon sent invalid JSON"));
          return;
        }
        ...
        if (!handshakeSettled) {
          ...
          failHandshake(new Error("WakuWaku daemon sent an invalid handshake response"));
      ...
      socket.addEventListener("error", () => {
        failHandshake(new Error("WakuWaku daemon connection failed"));
      });
      socket.addEventListener("close", () => {
        if (generation !== this.connectionGeneration) return;
        failHandshake(new Error("WakuWaku daemon disconnected during handshake"));
        this.markDisconnected(new Error("WakuWaku daemon disconnected"));
      });
    });
  }
  ...
  request(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error("timed out waiting for WakuWaku daemon"));
  ...
  async queryTrajectory(
    ...
    const payload = await this.request({ type: "queryTrajectory", query }, sessionId, NIL_UUID);
    if (payload.type !== "trajectory") {
      throw new Error("WakuWaku daemon returned an invalid trajectory response");
    }
  ...
  subscribeTrajectory(
    ...
    return this.subscribe(sessionId, runtimeId, (event) => {
      if (event.payload.type === "trajectory") {
        listener(event.payload.update, event);
      }
    });
  }
  ...
  private requireSocket(): WebSocketLike {
    if (this.state !== "connected" || !this.socket || this.socket.readyState !== OPEN) {
      throw new Error("WakuWaku daemon is disconnected");
    }
    return this.socket;
  }
