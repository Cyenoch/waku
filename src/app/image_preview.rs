...
impl Waku {
    ...
    pub(super) fn image_for_reference(
        ...
        let attachment_reference =
            reference.starts_with(wakuwaku_protocol::attachments::ATTACHMENT_SCHEME);
        if !wakuwaku_protocol::blob::is_reference(reference) && !attachment_reference {
            return None;
        }
        ...
        cx.spawn(async move |waku, cx| {
            let image = cx
                .background_executor()
                .spawn(async move {
                    wakuwaku_client::persistence::read_remote_reference(
                        &fetch_reference,
