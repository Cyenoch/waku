                    ...
                    let commands = cx
                        ...
                        .spawn(async move {
                            match workspace.request(
                                wakuwaku_client::WorkspaceOperation::DiscoverSlashCommands {
                                    provider: provider.clone(),
                                    project_root: path,
                                },
                            ) {
                                Ok(wakuwaku_client::WorkspaceResult::SlashCommands {
                                    commands,
                                }) => commands,
                                Ok(_) | Err(_) => Vec::new(),
                            }
            ...
            Query::Missing(token) => {
                ...
                cx.spawn(async move |waku, cx| {
                    let files = cx
                        ...
                        .spawn(async move {
                            match workspace.request(
                                wakuwaku_client::WorkspaceOperation::ListProjectFiles {
                                    root: path,
                                    cap: FILE_INDEX_CAP,
                                },
                            ) {
                                Ok(wakuwaku_client::WorkspaceResult::ProjectFiles { entries }) => {
                                    entries
                                }
                                Ok(_) | Err(_) => Vec::new(),
                            }
